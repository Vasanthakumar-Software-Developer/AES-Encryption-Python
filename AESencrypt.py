...
fileout=open("KEY.txt","w")
# set passphrase to be a 16 characters, 16 characters * 8 bits = 128 bits strength
 
# using random.choices()
# generating random strings
res = ''.join(random.choices(string.ascii_uppercase +string.ascii_lowercase+string.punctuation+string.digits, k=random.randint(30,50)
))
key=str(res).replace('"',"")

PassPhrase2=key
if(len(PassPhrase2)%16!=0):
    if(len(PassPhrase2)<16):
        Pass=PassPhrase2
        k=0
        while(k<(16-len(PassPhrase2))/len(PassPhrase2)):
            Pass=Pass+PassPhrase2
        PassPhrase2=Pass+PassPhrase2[0:16-len(Pass)]
    if(len(PassPhrase2)>16):
        PassPhrase2=PassPhrase2+PassPhrase2[0:16-len(PassPhrase2)%16]
file=open(sys.argv[1], "r")
message=(file.read())
    # print("Inside your plaintext message is:\n%s\n" % message)
file.close()
print("PlainText:"+message)
print("Strong Cyclic Key:"+PassPhrase2)
fileout.write(PassPhrase2)
i=1
n=int(len(PassPhrase2)/16)
while(i<=n):
    PassPhrase=PassPhrase2[16*(i-1):16*i]
    ...
    Filout=open(sys.argv[1],"w")
    Filout.write(File.read())
    Filout.close()
    File.close()
    i+=1
file2=open(sys.argv[2], "r")
print("The Ciphertext of given plaintext:\n%s\n" % file2.read())
file2.close()